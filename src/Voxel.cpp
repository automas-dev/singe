...
namespace Tom::s3e {
    ...
    BlockStyle::BlockStyle(const UV & north,  const UV & south, const UV & east, const UV & west, const UV & bottom,
                           const UV & top)
        : north(north),
          south(south),
          east(east),
...
namespace Tom::s3e {
    ...
    Cube::Cube(const glm::vec3 & origin, const BlockStyle::Ptr & style) : style(style) {
        auto & p = origin;
